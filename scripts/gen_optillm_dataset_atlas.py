...
# OptILM approaches
DEFAULT_APPROACHES = ["none", "mcts", "bon", "moa", "rto", "z3", "self_consistency", "pvg", "rstar", "cot_reflection", "plansearch", "leap", "re2"]
...
async def generate_response(prompt: str, approach: str) -> Dict[str, Any]:
    ...
    if approach == "none":
        ...
        response = await client.chat.completions.create(
            model=f"{approach}-/workspace/Arcee-Atlas",  # Assuming OptILM uses this naming convention
            messages=[{"role": "user", "content": prompt}],
        )
        return {
            "content": response.choices[0].message.content,
...
async def process_sample(sample: Dict[str, Any], approaches: List[str]) -> Dict[str, Any]:
...
async def generate_dataset(num_samples: int, output_file: str, approaches: List[str]):
    ...
    with open(output_file, "w") as f:
        for sample in tqdm(dataset.select(range(num_samples)), total=num_samples):
            try:
                result = await process_sample(sample, approaches)
...
def main():
    ...
    parser.add_argument("--approaches", nargs="+", default=DEFAULT_APPROACHES, help="List of approaches to use")
    args = parser.parse_args()

    asyncio.run(generate_dataset(args.num_samples, args.output_file, args.approaches))
